...
if (!function_exists('form_input_bs')) {
     ...
     * @param mixed $validation - hlášky pro validní formulář a invalidní formulář
     * @param boolean $notation - jestli se mají před uvozovky přidávat \ (pokud to cchi použít v javascriptu, dát false)
     */
    function form_input_bs($data = '',  string $label = '', string $type = 'text', $floating = true, array $validation = ['valid' => '', 'invalid' => ''], string $bs = 'mb-3',  $notation = true): string
    {
        if ($notation) {
            $quot = "\"";
            $endL = "\n";
...
if (! function_exists('form_dropdown_bs')) {
    ...
    function form_dropdown_bs($name = '', $options = [], $extra = '', string $bs = '', string $label = '', $disabled = [], $selected = [], $hidden = [], $notation = true): string
    {
        ...
        //test if class in extra, if not add it, if yes, test, if form-contol class added
        $test = array_key_exists('class', $extra);
        if (!$test) {
            $extra['class'] = 'form-select';
        } else {
            if (!str_contains($extra['class'], 'form-select')) {
                $extra['class'] .= ' form-select';
            }
        }
        ...
        //počíteční select
        $form     .= '<select name=' . $quot . $name . $quot  . $extra2  . '>' . $endL;
        //options
...
if (!function_exists('filter_input_bs')) {
    ...
    function filter_input_bs($input)
    {
        ...
        $form =
            ...
            $result = div(['class' => 'col-4']) . form_input_bs($dataFilter, $input, 'Filtrovat',) . "</div>";
